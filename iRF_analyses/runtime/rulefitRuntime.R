...
options(cores=32)
set.seed(32)
rfhome <- "~/iRF_analyses/runtime/rulefit/" #directory for rulefit package
source(paste0(rfhome, 'library_rulefit.r'))
...
for (i in 1:length(pseq)){
  ...
  save(runtime, auroc, file='runtime.rulefit.RData')
  print(paste0('p: ', pseq[i], '; time: ', runtime[i]))
}
